/*<ATGCOPYRIGHT>
 * Copyright (C) 2007-2009 Art Technology Group, Inc.
 ...
 *
 * @version $Id: //product/AssetUI/version/10.0.3/src/web-apps/AssetManager/scripts/gridServerModel.js#2 $$Change: 651448 $
...
dojo.declare("atg.assetmanager.CollectionEditorData", dojox.grid.data.Dynamic, {
  ...
  requestRows: function(pRowIndex, pCount) {
    var params = {
      startIndex: pRowIndex,
      endIndex: (pRowIndex + pCount)
    };
    var cbs = {
      callback: dojo.hitch(this, this._requestRowsCallback, pRowIndex)
      // no errback:
    };
  ...
  _requestAddItemsCallback: function(pIdArray, pData) {
    ...
    if (pData.numDuplicates > 0) {
      var msg = this.duplicatesDetectedMessage;
      msg = msg.replace('{0}', pData.numItemsAdded);
      msg = msg.replace('{1}', pData.numDuplicates);
      parent.messages.addAlert(msg);
    }
