...
function atgCalendar_changeDate(calendarInstance) {
  ...
  if (nodeType == "DIV") {
     ...
     */
    var tempHtml = atgCalendar_getCalendarHTML(calendarInstance);
    var orginalNode = containerObject;
     ...
     */
    var tempHtml = atgCalendar_getCalendarHTML(calendarInstance);
    var orginalNode = iframeBodyObject;
    var clonedNode = orginalNode.cloneNode(true);
    var parentNode = orginalNode.parentNode;
...
function atgCalendar_showPreviousMonth(calendarInstance) {
	...
	calendarInstance.displayDate = displayDate;
         ...
         */
        var tempHtml = atgCalendar_getCalendarHTML(calendarInstance);
        var orginalNode = atgCalendar_findElementById(calendarInstance.containerId);
        var clonedNode = orginalNode.cloneNode(true);
        var parentNode = orginalNode.parentNode;
...
function atgCalendar_showCalendar() {
	if ((calendarInstance.curtainId != null) && (calendarInstance.curtainId != ""))
	{
    	...
    	if (calendarInstance.iFrameId != null && calendarInstance.iFrameId != "") {
    	...
    	atgCalendar_debug(" windowHeight="+windowHeight);
      ...
      if (scrollHeight > windowHeight)
			...
			curtainObject.style.zIndex = 100;
		}
	}
  ...
  if (calendarInstance.iFrameId != null && calendarInstance.iFrameId != "") {
	...
	{
		calendarObject.style.display="block";
		calendarObject.style.zIndex=999;
	}
...
function atgCalendar_resizeIFrameContainer() {
  if (calendarInstance.iFrameId != null && calenderInstance.iFrameId != "") {
	  var calendarObject = atgCalendar_findElementById(calendarInstance.containerId);
	  var domWidth = calendarObject.offsetWidth;
	  var domHeight = calendarObject.offsetHeight;
...
function atgCalendar_getOverlapObject(positionObject) {
	...
	if (positionObject.isIFrame) {
	  ...
	  if (document.documentElement)
	  {
	  	if (document.documentElement.offsetWidth > windowWidth) windowWidth = document.documentElement.offsetWidth;
	  	if (document.documentElement.offsetHeight > windowHeight) windowHeight = document.documentElement.offsetHeight;
	  }
	}
