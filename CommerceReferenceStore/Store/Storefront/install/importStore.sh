...
if [ $switching = "true" ]; then
  if [ $i18n = "true" ]; then
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore -m Store.EStore.International -repository /atg/commerce/catalog/ProductCatalog -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-international-catalog.xml
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore -repository /atg/commerce/pricing/priceLists/PriceLists -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-pricelists.xml
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore -repository /atg/store/stores/StoreRepository -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/stores.xml
  else 
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore -repository /atg/commerce/catalog/ProductCatalog -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-no18n-catalog.xml
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore -repository /atg/commerce/pricing/priceLists/PriceLists -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-pricelists.xml
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore -repository /atg/store/stores/StoreRepository -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/stores.xml
  fi
fi
...
if [ $store = "true" ] && [ $switching != "true" ]; then
  if [ $i18n = "true" ]; then
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore -m Store.EStore.International -repository /atg/commerce/catalog/ProductCatalog -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-international-catalog.xml
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore -repository /atg/commerce/pricing/priceLists/PriceLists -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-pricelists.xml
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore -repository /atg/commerce/inventory/InventoryRepository -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-inventory.xml
    ...
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore -repository /atg/seo/SEORepository -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/seo-tags.xml
...
if [ $search = "true" ]; then
  if [ $i18n = "true" ]; then
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore -m Store.EStore.International -m DCS.Search.CustomCatalogs -repository /atg/search/repository/RefinementRepository -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-international-facets.xml
    ...
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore -m DCS.Search.CustomCatalogs -repository /atg/search/repository/RefinementRepository -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-facets.xml
