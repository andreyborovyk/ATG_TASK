...
if [ $store = "true" ]; then
  if [ $i18n = "true" ]; then
    ...
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore.Versioned -m Store.EStore.International -repository /atg/commerce/catalog/ProductCatalog -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-versioned-catalog.xml -workspace initiali18nCatalog -comment initiali18nCtlg -user publishing
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore.Versioned -repository /atg/commerce/pricing/priceLists/PriceLists -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-pricelists.xml -workspace initialPriceList -comment initialPriceList -user publishing
    $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore.Versioned -repository /atg/store/stores/StoreRepository -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/stores.xml -workspace initialpStore -comment initialStore -user publishing
    ...
    $DYNAMO_HOME/bin/startSQLRepository -m DAS.Versioned -m Store.EStore.Versioned -repository /atg/seo/SEORepository -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/seo-tags.xml -workspace initialSEOTags -comment initialSEOTags -user publishing
  else 
   echo "***Import Data for Version Store Non International"
   $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore.Versioned -repository /atg/commerce/catalog/ProductCatalog -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-versioned-no18n-catalog.xml -workspace initialPrdctCtlg -comment initialPrdctCtlg -user publishing
   $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore.Versioned -repository /atg/commerce/pricing/priceLists/PriceLists -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/out-pricelists.xml -workspace initialPriceList -comment initialPriceList -user publishing
   $DYNAMO_HOME/bin/startSQLRepository -m Store.EStore.Versioned -repository /atg/store/stores/StoreRepository -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/stores.xml -workspace initialStore -comment initialStore -user publishing
   $DYNAMO_HOME/bin/startSQLRepository -m DCS-UI -m Store.EStore.Versioned -repository /atg/web/viewmapping/ViewMappingRepository -import $DYNAMO_HOME/../CommerceReferenceStore/Store/EStore/Versioned/install/data/viewmapping.xml -workspace initialVwMapping -comment initialVwMapping -user publishing
   $DYNAMO_HOME/bin/startSQLRepository -m DAS.Versioned -m Store.EStore.Versioned -repository /atg/seo/SEORepository -import $DYNAMO_HOME/../CommerceReferenceStore/Store/Storefront/data/seo-tags.xml -workspace initialSEOTags -comment initialSEOTags -user publishing
  fi
