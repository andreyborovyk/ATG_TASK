 ...
 * parameter specifies the number of columns the array set should be
 * partitioned into.  The array is striped across the columns.
 *
 * <p>The <i>output</i> parameter is rendered for each "cell" in the
 * table.  Note that if the range size is not a multiple of the number of
 ...
 *
 * <p>A complete description of the parameters to the TableRange droplet
 * are:
 *
 * <dl>
 *
 ...
 * time the <i>output</i> parameter is rendered.  If the range size is not
 ...
 * @version $Id: //product/DAS/version/10.0.3/Java/atg/droplet/TableRange.java#2 $$Change: 651448 $
 * @updated $DateTime: 2011/06/07 13:55:45 $$Author: rbarbier $
 **/

public class TableRange extends DynamoServlet {
  //-------------------------------------
  // Constants
  //-------------------------------------
  ...
  "$Id: //product/DAS/version/10.0.3/Java/atg/droplet/TableRange.java#2 $$Change: 651448 $";
       ...
       throws ServletException, IOException
  {
    ...
    if (arrayParam == null) {
      ...
      if (isLoggingDebug())
        logDebug(ResourceUtils.getMsgResource("noArray", MY_RESOURCE_NAME,
                sResourceBundle));
  ...
  logError(ResourceUtils.getMsgResource("illegalHowMany", MY_RESOURCE_NAME,
    ...
    if (numColumnsParam == null) {
      if (isLoggingError())
  logError(ResourceUtils.getMsgResource("noNumColumns", MY_RESOURCE_NAME,
                sResourceBundle));
      return;
    }
    try {
  ...
  if (isLoggingError()) {
    ...
    logError(ResourceUtils.getMsgResource("illegalNumColumns", MY_RESOURCE_NAME,
            sResourceBundle, args));
  }
  return;
      }
  ...
  logError(ResourceUtils.getMsgResource("illegalNumColumns", MY_RESOURCE_NAME,
    ...
    if (arrayParam.getClass().isArray()) {
      serviceArray(arrayParam, start, howMany, numColumns, indexName, elementName, pRequest, pResponse);
    }
    // array param is a List
    else if (arrayParam instanceof List) {
      List list = (List) arrayParam;
      serviceList(list, start, howMany, numColumns, indexName, elementName, pRequest, pResponse);
    }
    // array param is a Collection
    else if (arrayParam instanceof Collection) {
      Collection collection = (Collection)arrayParam;
      serviceCollection(collection, start, howMany, numColumns, indexName, elementName, pRequest, pResponse);
    }
    // array param is a Iterator
    else if (arrayParam instanceof Iterator) {
      Iterator iterator = (Iterator)arrayParam;
      serviceIterator(iterator, start, howMany, numColumns, indexName, elementName, pRequest, pResponse);
    }
    // array param is an Enumeration
    else if (arrayParam instanceof Enumeration) {
      Enumeration enumeration = (Enumeration) arrayParam;
      serviceEnumeration(enumeration, start, howMany, numColumns, indexName, elementName, pRequest, pResponse);
    }
    // array param is a Map
    else if (arrayParam instanceof Map) {
      Map map = (Map)arrayParam;
      serviceMap(map, start, howMany, numColumns, indexName, elementName, pRequest, pResponse);
    }
    // array param is a Dictionary
    else if (arrayParam instanceof Dictionary) {
      Dictionary dictionary = (Dictionary) arrayParam;
      serviceDictionary(dictionary, start, howMany, numColumns, indexName, elementName, pRequest, pResponse);
    }
   ...
   **/
  protected void serviceArray(Object pArray, int pStart, int pHowMany, int pNumColumns,
                              String pIndexName, String pElementName,
                              DynamoHttpServletRequest pRequest,
                              DynamoHttpServletResponse pResponse)
       throws ServletException, IOException
  {
    serviceRange(pArray, pStart, pHowMany, pNumColumns, pIndexName, pElementName, pRequest, pResponse);
  }
   ...
   * @exception IOException if an error occurred reading data from the request or writing data to the response
   **/
  protected void serviceList(List pList, int pStart, int pHowMany, int pNumColumns,
                             String pIndexName, String pElementName,
                             DynamoHttpServletRequest pRequest,
                             DynamoHttpServletResponse pResponse)
       throws ServletException, IOException
  {
    serviceRange(pList, pStart, pHowMany, pNumColumns, pIndexName, pElementName, pRequest, pResponse);
  }
   ...
   * @exception IOException if an error occurred reading data from the request or writing data to the response
   **/
  protected void serviceCollection(Collection pCollection, int pStart, int pHowMany, int pNumColumns,
                                   String pIndexName, String pElementName,
                                   DynamoHttpServletRequest pRequest,
                                   DynamoHttpServletResponse pResponse)
       throws ServletException, IOException
  {
    ...
    serviceArray(array, pStart, pHowMany, pNumColumns, pIndexName, pElementName, pRequest, pResponse);
  }
   ...
   * @exception IOException if an error occurred reading data from the request or writing data to the response
   **/
  protected void serviceIterator(Iterator pIterator, int pStart, int pHowMany, int pNumColumns,
                                   String pIndexName, String pElementName,
                                   DynamoHttpServletRequest pRequest,
                                   DynamoHttpServletResponse pResponse)
       throws ServletException, IOException
  {
    if (!pIterator.hasNext())
      ...
      serviceList(list, pStart, pHowMany, pNumColumns, pIndexName, pElementName, pRequest, pResponse);
    }
  }
   ...
   * @exception IOException if an error occurred reading data from the request or writing data to the response
   **/
  protected void serviceEnumeration(Enumeration pEnumeration, int pStart, int pHowMany, int pNumColumns,
                                    String pIndexName, String pElementName,
                                    DynamoHttpServletRequest pRequest,
                                    DynamoHttpServletResponse pResponse)
       throws ServletException, IOException
   ...
   * @exception IOException if an error occurred reading data from the request or writing data to the response
   **/
  protected void serviceMap(Map pMap,  int pStart, int pHowMany, int pNumColumns,
                            String pIndexName, String pElementName,
                            DynamoHttpServletRequest pRequest,
                            DynamoHttpServletResponse pResponse)
       throws ServletException, IOException
  {
    if (pMap.isEmpty()) {
      ...
      if (entrySet != null) {
        ...
        serviceArray(entries, pStart, pHowMany, pNumColumns, pIndexName, pElementName, pRequest, pResponse);
      }
    }
  }
   ...
   * @exception IOException if an error occurred reading data from the request or writing data to the response
   **/
  protected void serviceDictionary(Dictionary pDictionary, int pStart, int pHowMany, int pNumColumns,
                                   String pIndexName, String pElementName,
                                   DynamoHttpServletRequest pRequest,
                                   DynamoHttpServletResponse pResponse)
       throws ServletException, IOException
  {
    if (pDictionary.isEmpty()) {
      ...
      if (pStart > length)
        ...
        Range.setPrevNextParameters(pStart, pHowMany, length, pRequest);
        pRequest.serviceLocalParameter(OUTPUT_START, pRequest, pResponse);
        int i = pStart - 1, irow = 0, icol = 0;
        int end = Math.min(i + pHowMany, length);
  ...
  void serviceRange(Object pArrayParam, int pStart, int pHowMany, int pNumColumns,
                    String pIndexName, String pElementName,
                    DynamoHttpServletRequest pRequest,
                    DynamoHttpServletResponse pResponse)
       throws ServletException, IOException
  {
    ...
    if ((length == 0) || (pStart > length)) {
      ...
      for ( ; i < end; i++) {
        ...
        pRequest.setParameter(COUNT, NumberTable.getInteger(i + 1));
        if (pArrayParam instanceof List) {
          setElementParameter(((List)pArrayParam).get(i), pElementName, pRequest, pResponse);
        }
        else {
