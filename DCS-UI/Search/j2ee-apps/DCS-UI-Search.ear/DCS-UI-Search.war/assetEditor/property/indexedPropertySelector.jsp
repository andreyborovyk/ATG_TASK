<%--
  ...
  @version $Id: //product/DCS-UI/version/10.0.3/search/src/web-apps/DCS-UI-Search/assetEditor/property/indexedPropertySelector.jsp#2 $$Change: 651448 $
...
<c:if test="${!readonlyPropName}">
  <script type="text/javascript">
    ...
    function setPropertyType()
    {
      ...
      var actionURI = document.getElementById("assetForm").action;
      actionURI += "&property=";
      actionURI += propertyName;
      actionURI += "&propertyType=";
      for (var j=0; j<dropDownArray.length; j++)
      {
        if (propertyName == dropDownArray[j].propertyName)
        {
          actionURI += dropDownArray[j].propertyType;
        }
      }
      document.getElementById("assetForm").action = actionURI;
...
<c:choose>
      ...
      <c:choose>
        ...
        <c:otherwise>
          ...
          <dspel:input type="hidden"
                       id="propertyDsp" name="propertyDsp"
  ...
  <tr>
    ...
    <td>
      <div id="propertyTypeDisplay">
        <c:if test="${!empty param.propertyType}">
          <dspel:setvalue bean="${formHandlerPath}.value.${formTypeProperty}" value="${param.propertyType}"/>
        </c:if>
