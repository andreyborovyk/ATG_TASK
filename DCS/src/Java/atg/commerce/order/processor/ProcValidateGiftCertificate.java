 ...
 * time.  This processor verifies that all required fields in a GiftCertificate
 ...
 * @version $Id: //product/DCS/version/10.0.3/Java/atg/commerce/order/processor/ProcValidateGiftCertificate.java#2 $$Change: 651448 $
...
{  
  ...
  public static String CLASS_VERSION =
      "$Id: //product/DCS/version/10.0.3/Java/atg/commerce/order/processor/ProcValidateGiftCertificate.java#2 $$Change: 651448 $";
  ...
  public int runProcess(Object pParam, PipelineResult pResult) throws Exception
  {
    ...
    if (isLoggingDebug())
      logDebug("Validating one GiftCertificate of type " + paymentGroup.getPaymentGroupClassType());
    
    validateGiftCertificateFields((GiftCertificate)paymentGroup, pResult, resourceBundle);
  ...
  protected void validateGiftCertificateFields(
    GiftCertificate pGiftCert, PipelineResult pResult, ResourceBundle resourceBundle)
  {
    // First check to make sure we have a gift certificate number.  If not, then
    // there's no sense in proceeding.

    String gcNumber = pGiftCert.getGiftCertificateNumber();
    
    if (isEmpty(gcNumber)) {
      String msg = resourceBundle.getString("GiftCertificateNumberMissing");
      addHashedError(pResult, PipelineConstants.MISSINGGIFTCERTIFICATENUMBER, pGiftCert.getId(), msg);
      return;
    }

    // Now verify that the gift certificate number we got isn't a fake.
    
    try
    {
      if (getClaimableManager().claimItem(gcNumber) == null) {
        String msg = resourceBundle.getString("GiftCertificateNumberNotFound");
        addHashedError(pResult, PipelineConstants.GIFTCERTIFICATENUMBERNOTFOUND, pGiftCert.getId(), msg);
      }
    }
    catch (ClaimableException c) {
      addHashedError(pResult, PipelineConstants.GIFTCERTIFICATENUMBERCLAIMEXCEPTION, pGiftCert.getId(), c.getMessage());
    }
  }
  ...
  String mLoggingIdentifier = "ProcValidateGiftCertificate";
