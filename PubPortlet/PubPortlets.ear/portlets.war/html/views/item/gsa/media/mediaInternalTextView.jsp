<%--
  Default repository file asset edit view
...
<!-- Begin BIZUI's /views/item/gsa/media/mediaInternalTextView.jsp -->
...
<c:forEach items="${cattedItems}" var="propertyDescriptor" varStatus="loopStatus">
  ...
  <c:if test="${propertyDescriptor.class.name != 'atg.epub.pws.taglib.CategoryPropertyDescriptor'}">
    <c:if test="${propertyDescriptor.name == 'name' || propertyDescriptor.name == 'description' || propertyDescriptor.name == 'parentFolder' || propertyDescriptor.name == 'path' || propertyDescriptor.name == 'startDate' || propertyDescriptor.name == 'endDate'  }">
...
<!-- End BIZUI's /views/item/gsa/media/mediaInternalTextView.jsp -->
<%-- @version $Id: //product/BIZUI/version/10.0.3/jsp-src/views/item/gsa/media/mediaInternalTextView.jsp#2 $$Change: 651448 $--%>
