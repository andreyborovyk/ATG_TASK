...
<tr>
  ...
  <c:choose>
    <c:when test="${pvIsDisabled}">
      <dspel:textarea iclass="formElementCustom assetEdit" disabled="true"
                      id="propertyValue_${mpv.uniqueId}"
                      rows="${mpv.attributes.rows}"
               cols="${mpv.attributes.columns}"
                      bean="${mpv.formHandlerProperty}"/>
    </c:when>
    <c:otherwise>
      <dspel:textarea iclass="formElementCustom assetEdit"
                      id="propertyValue_${mpv.uniqueId}"
                      rows="${mpv.attributes.rows}"
               cols="${mpv.attributes.columns}"
                      bean="${mpv.formHandlerProperty}"/>
...
<script language="JavaScript">
  ...
  <c:if test="${'true' != mpv.attributes.disableFormSaver}">
...
<%-- @version $Id: //product/BIZUI/version/10.0.3/jsp-src/views/property/bigstring/textAreaEdit.jsp#2 $$Change: 651448 $--%>
