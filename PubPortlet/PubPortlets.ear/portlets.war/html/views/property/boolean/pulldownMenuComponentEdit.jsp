...
<script language="JavaScript">
  ...
  function collectionDrawItem_<c:out value="${mpv.uniqueId}"/> (fieldId, fieldValue, fieldDisplayName) {
    ...
    tempHTML += '<select <c:out value="${pvIsDisabled}"/> id="collectionValue_' + fieldId + '">';

    if ( fieldValue == "true" || fieldValue == "1") {
      tempHTML += '<option value="1" selected="selected">true</option>';
      tempHTML += '<option value="0">false</option>';
    } else {
      tempHTML += '<option value="1">true</option>';
      tempHTML += '<option value="0" selected="selected">false</option>';
    }
...
<%-- @version $Id: //product/BIZUI/version/10.0.3/jsp-src/views/property/boolean/pulldownMenuComponentEdit.jsp#2 $$Change: 651448 $--%>
