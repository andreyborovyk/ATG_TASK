<%--
  ...
  @version $Id: //product/BIZUI/version/10.0.3/jsp-src/views/property/item/nestedEdit.jsp#2 $$Change: 651448 $
  ...
  <table border="0" cellpadding="0" cellspacing="3">
    <tr>
      ...
      <td>
            ...
            <pws:getItemSubTypes var="subtypes"
              ...
              <c:forEach items="${subtypes}" var="subtype">
                ...
                <pws:getMappedRepository var="mappedRepository" 
                  repositoryName="${subtype.repository.repositoryName}"/>
        ...
        <c:if test="${mpv.attributes.editItemButtonText != null}">
          <c:set var="pvEditItemText" value="${mpv.attributes.editItemButtonText}"/>
        </c:if>
        <input type="button" value="<c:out value="${pvEditItemText}"/>"  class="formElementCustom"
          onmousedown="<c:out value="${mpv.uniqueId}"/>_pick();"/>

        <%-- Show the clear button --%>
        <c:set var="pvClearItemText" value="clear link"/>
        <c:if test="${mpv.attributes.clearItemButtonText != null}">
          <c:set var="pvClearItemText" value="${mpv.attributes.clearItemButtonText}"/>
        </c:if>

        <input type="button" value="<c:out value="${pvClearItemText}"/>" class="formElementCustom"
          onmousedown='<c:out value="${mpv.uniqueId}"/>_set( { id: "null", displayName: ""} );'/>
    ...
    <tr>
      <td>&nbsp;</td>
      <td>
        <dspel:include page="/html/views/item/gsa/nestedViewContainer.jsp"/>
      </td>
    </tr>
