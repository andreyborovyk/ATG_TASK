...
<tr>
  ...
  <div>
    ...
    <%-- Is this a map of items?  or of primitives? --%>
    <c:set var="collValueProperty" value="${mpv.formHandlerProperty}.keysAndValues"/>
    <c:if test="${!empty mpv.componentRepositoryItemDescriptor}">
      <c:set var="collValueProperty" value="${mpv.formHandlerProperty}.keysAndRepositoryIds"/>
    </c:if>
    ...
    <%-- Note that we need to start off with one bogus reference to the map
         to get the Nucleus machinery going.  This will get removed during the
         next DIV rewrite --%>
    <div id="collectionHidden_<c:out value='${mpv.uniqueId}'/>">
      <dspel:input type="hidden" value="dummy=null" bean="${collValueProperty}"/>
  ...
  <script language="JavaScript">
    ...
    var collData = collectionRegister( "<c:out value="${mpv.uniqueId}"/>", "map", 
      "<c:out value="${collValueProperty}"/>", "<c:out value="${collNullProperty}"/>");
  ...
  <%-- Add any items already within the map --%>
  ...
  <c:forEach items="${arrayValues}" var="arrayValue" varStatus="status">
    <c:choose>
      <c:when test="${!empty mpv.componentRepositoryItemDescriptor}">
        ...
        <script language="JavaScript">
          collectionDoAddItem( "<c:out value="${mpv.uniqueId}"/>",
                               "<dspel:valueof bean="${mpv.formHandlerProperty}.keys[${status.index}]"/>",
...
<script language="JavaScript">
  ...
  function get_<c:out value="${mpv.uniqueId}"/> ()
  {
    ...
    // Encode the data as a '|' delimited list of key=value=displayName
    ...
    for ( var x in collData.items) {
      tmpStr += encodeURIComponent(collData.items[x].key) + '=' +
                encodeURIComponent(collData.items[x].value) + '=' +
  ...
  function set_<c:out value="${mpv.uniqueId}"/> ( value)
  {
    ...
    for ( var x in pairs) {
      if ( pairs[x] != '') {
        var kvd = pairs[x].split('=');
        var key = decodeURIComponent(kvd[0]);
        var value = decodeURIComponent(kvd[1]);
        var displayName = decodeURIComponent(kvd[2]);
        var assetURI = decodeURIComponent(kvd[3]);

        collectionAddItem( '<c:out value="${mpv.uniqueId}"/>', key, value, displayName, assetURI);
      }
    }
...
<%-- @version $Id: //product/BIZUI/version/10.0.3/jsp-src/views/property/map/mapEdit.jsp#2 $$Change: 651448 $--%>
